...
class QubesInitialSetup(KickstartService):
    properties = (
        'system_vms', 'disp_firewallvm_and_usbvm', 'disp_netvm',
        'default_vms',
        'whonix_vms', 'whonix_default', 'usbvm', 'usbvm_with_netvm', 'skip',
        'allow_usb_mouse', 'allow_usb_keyboard',
        'vg_tpool', 'templates_to_install', 'default_template',
    )
    ...
    def __init__(self):
        ...
        if self.fedora_available:
            ...
            self.templates_aliases['fedora'] = 'Fedora %s' % \
                                               self.templates_versions['fedora']

        if self.debian_available:
            self.templates_versions['debian'] = get_template_version('debian')
            self.templates_aliases['debian'] = 'Debian %s' % \
                                               self.templates_versions['debian']

        if self.whonix_available:
            self.templates_versions['whonix'] = get_template_version(
                'whonix-ws')
            self.templates_aliases['whonix'] = 'Whonix %s' % \
    ...
    def publish(self):
        ...
        DBus.publish_object(QUBES_INITIAL_SETUP.object_path,
            QubesInitialSetupInterface(self))
    ...
    def setup_kickstart(self, data):
        ...
        # the need to copy this is stupid...
        for attr in ks_data.bool_options + (
                "templates_to_install", "default_template"):
